...
const HERO_MEDIA: HeroMediaItem[] = [
  ...
  {
    type: "image",
    uri: "https://images.unsplash.com/photo-1601000938259-9e92002320b2?auto=format&fit=crop&w=900&q=80",
    caption: "Expert carpenters",
  },
  {
    type: "image",
    uri: "https://images.unsplash.com/photo-1504148455328-c376907d081c?auto=format&fit=crop&w=900&q=80",
    caption: "Repairs & assembly",
  },
  {
    type: "image",
    uri: "https://images.unsplash.com/photo-1572021335469-31706a17aaef?auto=format&fit=crop&w=900&q=80",
...
// Carpenter sub-categories matching Urban Company
const CARPENTER_CATEGORIES = [
  {
    id: "cupboard-drawer",
    name: "Cupboard &\ndrawer",
    image: "https://images.unsplash.com/photo-1558997519-83ea9252edf8?auto=format&fit=crop&w=300&q=80",
  },
  {
    id: "kitchen-fittings",
    name: "Kitchen\nfittings",
...
// Grid Item Component
const GridItem = ({ item, onPress }: { item: typeof CARPENTER_CATEGORIES[0]; onPress: () => void }) => (
...
export default function CarpenterServiceScreen() {
  ...
  const cms = useCategoryCMS("carpenter");
  ...
  const dynSubCategories = useMemo(() => {
    if (cms.sub_categories && cms.sub_categories.length > 0) {
      return cms.sub_categories.map((s) => ({
        id: s.slug || s.id,
        name: s.name,
        image: s.image_url || ((CARPENTER_CATEGORIES)[0] as any)?.image || "",
        badge: s.badge || undefined,
        badgeColor: s.badge_color || undefined,
      }));
    }
    return CARPENTER_CATEGORIES;
  }, [cms.sub_categories]);

  const brandName = cms.category?.brand_name || cms.category?.name || "Carpenter";
  ...
  const promoLabel = promo?.label || "Free quote";
  const promoSubLabel = promo?.sub_label || "Same day visit";
  ...
  const rows: typeof CARPENTER_CATEGORIES[] = [];
        ...
        <View style={styles.heroWrapper}>
          ...
          <View style={styles.heroHeaderOverlay} pointerEvents="box-none">
            ...
            <View style={styles.heroHeaderRight}>
              <TouchableOpacity style={styles.heroIconBtn} onPress={() => require("expo-router").router.push("/search?category=carpenter")}><Search size={18} color="#000000" />
              </TouchableOpacity>
              <TouchableOpacity style={styles.heroIconBtn} onPress={() => Share.share({ message: "Check out carpenter services on Mfixit", url: "/category/carpenter" })}><Share2 size={18} color="#000000" />
              </TouchableOpacity>
            </View>
          </View>
        </View>
