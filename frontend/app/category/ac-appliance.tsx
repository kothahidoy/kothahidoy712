...
const AC_APPLIANCE_SUBCATEGORIES = [
  ...
  },
  {
    id: "microwave",
    name: "Microwave",
    image: "https://images.unsplash.com/photo-1574269909862-7e1d70bb8078?auto=format&fit=crop&w=200&q=80",
  },
  {
    id: "chimney",
    name: "Chimney &\nhob",
  ...
  },
  {
    id: "water-purifier",
    name: "Water\npurifier",
    image: "https://images.unsplash.com/photo-1564419320461-6870880221ad?auto=format&fit=crop&w=200&q=80",
  },
];

// Grid item component with explicit sizing
const GridItem = ({ item, onPress }: { item: typeof AC_APPLIANCE_SUBCATEGORIES[0]; onPress: () => void }) => (
  <TouchableOpacity 
...
export default function ACApplianceCategoryScreen() {
  const router = useRouter();


  // ── Pull CMS-managed content from Supabase ──
  const cms = useCategoryCMS("ac-appliance");
  ...
  const dynSubCategories = useMemo(() => {
    if (cms.sub_categories && cms.sub_categories.length > 0) {
      return cms.sub_categories.map((s) => ({
        id: s.slug || s.id,
        name: s.name,
        image: s.image_url || ((AC_APPLIANCE_SUBCATEGORIES)[0] as any)?.image || "",
        badge: s.badge || undefined,
        badgeColor: s.badge_color || undefined,
      }));
    }
    return AC_APPLIANCE_SUBCATEGORIES;
  }, [cms.sub_categories]);

  const brandName = cms.category?.brand_name || cms.category?.name || "AC & Appliance";
  ...
  const promoLabel = promo?.label || "Get 15% off";
  const promoSubLabel = promo?.sub_label || "On AC service";
  ...
  return (
    <SafeAreaView style={styles.root} edges={["top"]}>
        ...
        <View style={styles.heroWrapper}>
          ...
          <View style={styles.heroHeaderOverlay} pointerEvents="box-none">
            ...
            <View style={styles.heroHeaderRight}>
              <TouchableOpacity style={styles.heroIconBtn} onPress={() => require("expo-router").router.push("/search?category=ac-appliance")}><Search size={18} color="#000000" />
              </TouchableOpacity>
              <TouchableOpacity style={styles.heroIconBtn} onPress={() => Share.share({ message: "Check out ac-appliance services on Mfixit", url: "/category/ac-appliance" })}><Share2 size={18} color="#000000" />
              </TouchableOpacity>
            </View>
          </View>
        </View>
        ...
        <View style={styles.mainContent}>
          ...
          <View style={styles.titleSection}>
            ...
            <View style={styles.earliestBadge}>
              ...
              <Text style={styles.earliestTime}>Thu, 10:00 AM</Text>
            </View>
          </View>
      ...
      <View style={styles.bottomPromo}>
        <Tag size={14} color="#0891B2" />
        <Text style={styles.bottomPromoText}>₹150 off on AC service this summer</Text>
      </View>
    </SafeAreaView>
  );
}
