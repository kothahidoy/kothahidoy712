        ...
        {Object.entries(ALL_SERVICES).map(([categoryId, categoryData]) => (
          <View key={categoryId} onLayout={(e) => setSectionPositions(prev => ({ ...prev, [categoryId]: e.nativeEvent.layout.y }))}>
            ...
            {categoryData.services.map((service, index) => (
              <View key={service.id}>
                <ServiceCard service={service} quantity={serviceQty(service.id)} onAdd={() => handleServiceAddPress(service)} onRemove={() => removeOneForService(service.id)} onViewDetails={() => router.push(`/salon-women/service/${service.id}`)} />
